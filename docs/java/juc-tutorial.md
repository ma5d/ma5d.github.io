...
AQS 是 Java 并发包 java.util.concurrent 中的一个核心类，它提供了一种框架来构建各种同步器，如锁、信号量等。ReentrantLock 是一种可重入的互斥锁，它利用 AQS 的机制来实现线程的同步访问。

AQS 通过维护一个状态变量（state）和一个等待队列来管理线程的排队和阻塞。ReentrantLock 使用 AQS 的独占模式来实现锁的功能，其中 state 变量用来表示锁的状态（0表示未锁定，大于0表示锁定状态），等待队列则用来存放等待获取锁的线程。

当一个线程尝试获取 ReentrantLock 时，它首先会尝试通过 CAS（Compare-And-Swap）操作来改变状态变量，如果成功，则该线程获取了锁。如果失败，线程会被封装成一个节点加入到 AQS 的等待队列中，并且可能会被阻塞，直到它被唤醒并获得锁。

ReentrantLock 还提供了公平锁和非公平锁两种实现方式，区别在于线程获取锁的顺序。公平锁会按照线程请求锁的顺序来分配锁，而非公平锁则允许线程在锁可用时立即尝试获取锁，这可能会导致某些线程长时间得不到锁。
...
2. handle 类似于 thenAccept/thenRun 方法,是最后一步的处理调用,但是同时可以处理异常
```java
@Test
...
```java
@Test
public void combineRelate() throws ExecutionException, InterruptedException {
    final Integer[] num = {10};
    System.out.println("主线程开始");
    // 第一步加 10
    CompletableFuture<Integer> future = CompletableFuture.supplyAsync(() -> {
...
```java
@Test
public void combineUnRelate() throws ExecutionException, InterruptedException {
    final Integer[] num = {10};
    System.out.println("主线程开始");
    ...
    CompletableFuture<Object> future = job1.thenCombine(job2, 
            (BiFunction<Integer, Integer, List<Integer>>) (a, b) -> {
